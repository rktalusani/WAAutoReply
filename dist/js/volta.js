...
Volta.accordion = function () {
	...
	_accordion.prototype = {
		init: function(elementOrId, suppressClickHandler, triggerElem, isGroup, isStandard) {
			var element;
			...
			if(isStandard) {
				this._initStandard(element, suppressClickHandler, isGroup);
			} else {
				this._initJs(element, suppressClickHandler, triggerElem);
			}
		},
		_initStandard: function(element, suppressClickHandler, isGroup) {
			var self = this;

			this._parent = element;
			this._isGroup = isGroup;

			var allTriggers = this._parent.querySelectorAll('.' + _class.standard.trigger);
			this._triggers = [];
			allTriggers.forEach(function(trigger) {
				var parent = Volta._closest(trigger, '.' + _class.standard.container, _class.standard.container);

				if(parent && parent == self._parent) {
					self._triggers.push(trigger);
				}
			});

			this._panels = [];
			var allPanels = this._parent.querySelectorAll('.' + _class.standard.content);
			allPanels.forEach(function(panel) {
				var parent = Volta._closest(panel, '.' + _class.standard.container, _class.standard.container);
		...
		close: function(trigger) {
			if(this.trigger) {
				...
				this._closeStandardAccordion(trigger);
			}
		},
		_closeJsAccordion: function() {
			...
			this._content.classList.remove(_class.js.contentOpen);

			var self = this;
			this._content.addEventListener('transitionend', function closingTransitionEndEvent(event) {
				if (event.propertyName == 'height' && Volta._hasClass(self._content, _class.js.contentClosing)) {
					self._content.classList.remove(_class.js.contentClosing);
					self._content.style.height = '0px';
					self._content.removeEventListener('transitionend', closingTransitionEndEvent, false);
				}
			}, { passive: true, once: true });
		},
		_closeStandardAccordion: function(trigger) {
			var panel = this._getTriggersPanel(trigger);
			trigger.classList.remove(_class.standard.triggerActive);
			...
			panel.addEventListener('transitionend', function closingTransitionEndEvent(event) {
				if (event.propertyName == 'height' && Volta._hasClass(panel, _class.standard.contentClosing)) {
					...
					if(self._isGroup && self._isTriggerActive(trigger,  true)){
		...
		_isTriggerActive: function(trigger, match) {
			return (this._isGroup && this._activeGroupTrigger && (!match || this._activeGroupTrigger === trigger)) || Volta._hasClass(trigger, _class.standard.triggerActive);
		},
		_getTriggersPanel: function(trigger) {
		...
		_openJsAccordion: function() {
			...
			this._content.addEventListener('transitionend', function openingTransitionEndEvent(event) {
				if (event.propertyName == 'height' && Volta._hasClass(self._content, _class.js.contentOpening)) {
					self._content.style.height = 'auto';
					self._content.classList.remove(_class.js.contentOpening);
					self._content.classList.add(_class.js.contentOpen);
					self._content.removeEventListener('transitionend', openingTransitionEndEvent, false);
					self._isOpening = false;
				}
			}, { passive: true, once: true });
		},
		_openStandardAccordion: function(trigger) {
			if(this._isTriggerActive(trigger, false)) {
				this._closeStandardAccordion(this._activeGroupTrigger || trigger);
			}
			if(this._isGroup) {
			...
			trigger.classList.add(_class.standard.triggerActive);
			panel.classList.add(_class.standard.contentOpening);
			...
			panel.addEventListener('transitionend', function openingTransitionEndEvent(event) {
				if (event.propertyName == 'height' && Volta._hasClass(panel, _class.standard.contentOpening)) {
					panel.style.height = 'auto';
					panel.classList.remove(_class.standard.contentOpening);
					panel.classList.add(_class.standard.contentOpen);
		...
		toggle: function(trigger) {
			...
			if(this.trigger) {
				...
				this._toggleStandardAccordion(trigger);
			}
		},
		...
		_toggleStandardAccordion: function(triggerElement) {
			if(this._isTriggerActive(triggerElement, true)) {
	 ...
	 *  @return {Object}
	 */
	function create(elementOrId, suppressClickHandler, _trigger, isGroup, isStandard) {
		if(!elementOrId) {
			console.warn("Volta: no parameter supplied to accordion.create()");
		}
		var accordion = Object.create(_accordion.prototype, {});
		accordion.init(elementOrId, suppressClickHandler, _trigger, isGroup, isStandard);
